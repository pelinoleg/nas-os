...
REQUIRED_PKGS=(
  ...
  "git|the panel cannot version /var/lib/nas-os or update itself"
...
STACKS_DIR="/opt/stacks"           # INSTALLED stacks (what the panel deploys); services/ next to this script is the CATALOG, not the deployed set
...
NAS_CONFIG="/var/lib/nas-os"    # panel state, root-owned (see 0.6)
...
write_file() {
    ...
    if [ -f "$path" ]; then
        # Identical content = nothing to write and nothing to back up. Re-running a stage
        # is the NORMAL case (the panel calls `api <section>` on every save), and it used
        # to cost a copy every time — 17 of them piled up beside their originals in a
        # single afternoon. Not rewriting also leaves the mtime alone, which the caches
        # keyed on file time care about.
        if [ "$(cat "$path" 2>/dev/null)" = "$content" ]; then
            log "WRITE -> $path unchanged, skipped"
            return 0
        fi
        _bak_take "$path"
    fi
...
# Copies taken before overwriting go to ONE directory, not next to the original.
# /etc/update-motd.d and /etc/NetworkManager/dispatcher.d execute every file they
# contain, so a `*.bak.*` dropped beside the original is not an inert copy — it is a
# second live script (the SSH greeting printed itself twice until an explicit `rm` was
# bolted on for that one case). One directory also makes the pile prunable, which it
# never was while it was scattered across /etc.
BAKDIR="${NASW_BACKUP_DIR:-/var/backups/nas-os-wizard}"
BAK_KEEP="${NASW_BACKUP_KEEP:-3}"
_bak_take() {
    local f="$1" base old
    [ -f "$f" ] || return 0
    base="$(basename "$f")"
    mkdir -p "$BAKDIR" 2>/dev/null || return 0
    cp -a "$f" "$BAKDIR/${base}.$(date '+%Y%m%d-%H%M%S')" 2>/dev/null || return 0
    # keep the newest BAK_KEEP copies OF THIS FILE, drop the rest
    old="$(ls -1t "$BAKDIR/${base}."* 2>/dev/null | tail -n +$((BAK_KEEP + 1)))"
    [ -n "$old" ] && printf '%s\n' "$old" | while IFS= read -r o; do rm -f "$o"; done
    return 0
}

# nas_config_git_guard — keep the config repository from becoming a secret leak.
#
# $NAS_CONFIG is deliberately world-readable (0755) so the owner can look at their own
# configs, and the panel writes its secrets there with 0600 — but `git add -A` copies
# those bytes into .git/objects, which inherit the directory's openness and are created
# 0444. A chmod on the working file then protects nothing: the token, the credential
# store and the disaster card (it embeds the kopia repository password) stay readable by
# every local process, and by every past version too, because history does not forget.
# Found 2026-08-14: the display token, rotated that morning precisely because it had been
# world-readable, was recoverable from .git/objects by any unprivileged user — and the
# allow-list stored beside it still carried `reboot` and `poweroff`.
#
# Two locks, because either alone is one mistake away from failing: the repo itself is
# owner-only, and the secret-bearing files are never tracked in the first place.
nas_config_git_guard() {
    [ -d "$NAS_CONFIG/.git" ] || return 0
    if [ "$DRY_RUN" -eq 1 ]; then
        info "[DRY-RUN] lock $NAS_CONFIG/.git and keep secrets untracked"
        return 0
    fi
    chmod -R go-rwx "$NAS_CONFIG/.git" 2>/dev/null || true
    if ! grep -qs '^glance\.json$' "$NAS_CONFIG/.gitignore" 2>/dev/null; then
        {
            printf '# Secrets never belong in config history: objects under .git outlive a\n'
            printf '# chmod on the working file, and this repo sits in a world-readable directory.\n'
            printf 'glance.json\ncredentials.json\ndisaster-card.md\n'
        } >> "$NAS_CONFIG/.gitignore"
    fi
    # already committed on an older install? drop them from the index (the working files stay)
    git -C "$NAS_CONFIG" rm -q --cached glance.json credentials.json disaster-card.md \
        >/dev/null 2>&1 || true
}
...
commit_config() {
    ...
    chown -R root:root "$NAS_CONFIG" 2>/dev/null || true
    run git -C "$NAS_CONFIG" add -A
    run git -C "$NAS_CONFIG" -c user.email="nas@localhost" -c user.name="nas-wizard" commit -q -m "$msg" || true
}
...
stage_system() {
    ...
    # The panel's own state, and a git repository on top of it. Owned by root, not by
    # the login user: nas-web runs as root and starts before the pool is mounted, so its
    # state must not depend on a home directory — or on that user still existing after a
    # reinstall. Readable by everyone (0755) so the user can look; writing is the panel's job.
    if [ ! -d "$NAS_CONFIG" ]; then
        run mkdir -p "$NAS_CONFIG/scripts"
        run chown -R root:root "$NAS_CONFIG"
        run chmod 755 "$NAS_CONFIG"
    fi
    # git init + first commit
    if [ ! -d "$NAS_CONFIG/.git" ]; then
        run git -C "$NAS_CONFIG" init -q
        nas_config_git_guard          # before the first add -A, not after
        if [ "$DRY_RUN" -eq 0 ]; then
            if [ ! -f "$NAS_CONFIG/README.md" ]; then
                printf '# nas-config\n\nVersioned NAS configs (fstab snippets, snapraid.conf, docker-compose).\nGenerated by nas-wizard.sh.\n' \
                    > "$NAS_CONFIG/README.md"
            fi
            run git -C "$NAS_CONFIG" add -A
            run git -C "$NAS_CONFIG" -c user.email="nas@localhost" -c user.name="nas-wizard" commit -q -m "init nas-config" || true
        else
...
generate_deploy_script() {
    ...
    # This script is what nas-stacks.service runs at boot, so it must bring up what is
    # INSTALLED — /opt/stacks, where the panel deploys — and nothing else.
    #
    # It used to iterate services/ next to this script. That folder started life as "your
    # services live here" and has since become the panel's CATALOG of recipes (immich,
    # dockge, dozzle, myspeed, immich-kiosk…), so the boot unit would have run
    # `docker compose up -d` over every recipe in the repository: not "restore what was
    # running" but "install the whole catalogue", on every single boot. It never happened
    # here only because the unit itself was missing.
    write_file "$NAS_CONFIG/scripts/deploy.sh" <<DEPLOY
#!/usr/bin/env bash
# Auto-generated by nas-wizard. Idempotently brings up every INSTALLED stack.
# "Apply the desired state": docker compose up -d for each discovered file.
set -uo pipefail
COMPOSE_DIR="${STACKS_DIR}"
DC="docker compose"
if ! docker compose version >/dev/null 2>&1; then
...
DEPLOY
    ...
    run chown root:root "$NAS_CONFIG/scripts/deploy.sh" 2>/dev/null || true
}

# Container logs with no ceiling fill the system disk quietly: docker's json-file driver
# keeps every line a container ever wrote, forever, and logrotate does not touch them
# (docker owns those files). Measured here: 786 KB in 21 hours from ONE idle container —
# nothing today, unbounded by next year, and it lands on the same disk as the OS.
# Merged into daemon.json, never written over it: data-root may already be set there.
install_docker_logcaps() {
    run mkdir -p /etc/docker
    if [ "$DRY_RUN" -eq 1 ]; then
        info "[DRY-RUN] set log-driver=json-file, max-size=10m, max-file=3 in /etc/docker/daemon.json"
        return 0
    fi
    python3 - <<'PYJ'
import json
...
PYJ
    info "docker log cap: 10m x 3 per container (applies to containers created after the next docker restart)"
}

# journald with no ceiling takes 10% of the filesystem before it starts rotating — on a
...
# resolved to 172.18.0.1 (a docker network) instead of the box's real address, and the name
# simply did not work in the house. Publishing is scoped to the interface the default route
# uses. Re-run (`nas-wizard.sh api mdns`) if the box moves from Wi-Fi to cable or back.
install_mdns_scope() {
    ...
    [ -n "$iface" ] || { warn "mdns: no default route — leaving avahi on every interface"; return 0; }
    if [ "$DRY_RUN" -eq 1 ]; then
        info "[DRY-RUN] avahi allow-interfaces=$iface"
        return 0
    fi
    python3 - "$iface" <<'PYA'
import re, sys
f, iface = "/etc/avahi/avahi-daemon.conf", sys.argv[1]
try:
    txt = open(f).read()
except OSError:
    sys.exit(0)
line = "allow-interfaces=%s" % iface
if re.search(r"^\s*allow-interfaces=", txt, re.M):
    txt = re.sub(r"^\s*#?\s*allow-interfaces=.*$", line, txt, count=1, flags=re.M)
elif re.search(r"^\[server\]", txt, re.M):
...
PYA
    ...
    info "mdns: announcing on $iface only (was: every interface, docker bridges included)"
}
...
# Wi-Fi that NetworkManager does not own makes this ENTIRE branch a no-op. The Debian
# installer writes Wi-Fi into /etc/network/interfaces (ifupdown, mode 600, holds the PSK)
# and NetworkManager.conf then carries `[ifupdown] managed=false` — so every nmcli call
# above still "succeeds" while doing nothing at all, and the box looks protected by a
# fallback that cannot fire. A guard that silently protects nothing is worse than no
# guard, so this is said out loud: once per boot in the journal, once a day in the panel.
# (Teaching netguard to drive ifupdown is NOT the fix to reach for remotely: it runs every
# 30 s, and one `ifdown` on the only live link takes the box off the network.)
WUNMGD="${NAS_NETGUARD_WIFI_UNMANAGED:-/var/lib/nas-wizard/netguard.wifi-unmanaged}"
WUNMGD_BOOT="${NAS_NETGUARD_WIFI_UNMANAGED_BOOT:-/run/nas-netguard.wifi-unmanaged}"
wifi_present(){ [ -e "/sys/class/net/$WIFI" ]; }
wifi_managed(){
  wifi_present || return 1
  has_nm || return 1
  case "$(dev_state "$WIFI")" in ''|unmanaged) return 1 ;; esac
  return 0
}
wifi_unmanaged_warn(){
  local now stamp=0
  now="$(date +%s)"
  [ -e "$WUNMGD_BOOT" ] || {
    : > "$WUNMGD_BOOT" 2>/dev/null || true
    logj "$WIFI is NOT managed by NetworkManager (ifupdown owns it) — the Wi-Fi fallback in this guard does nothing"
  }
  [ -r "$WUNMGD" ] && read -r stamp < "$WUNMGD" 2>/dev/null
  case "$stamp" in ''|*[!0-9]*) stamp=0 ;; esac
  [ $(( now - stamp )) -lt 86400 ] && return 0
  mkdir -p "$(dirname "$WUNMGD")" 2>/dev/null || true
  printf '%s\n' "$now" > "$WUNMGD" 2>/dev/null || true
  notify "NAS: Wi-Fi fallback is not armed" \
    "$WIFI is configured outside NetworkManager, so this guard cannot switch to it if the wire dies. Moving the profile into NetworkManager drops the link while it happens — do it with a keyboard on the box, not over SSH." \
    1 link_changed "net:wifi-unmanaged"
}
if wifi_present && ! wifi_managed; then
  wifi_unmanaged_warn
else
  rm -f "$WUNMGD" "$WUNMGD_BOOT" 2>/dev/null || true
fi

if eth_healthy; then
  ACTIVE="$ETH"
  wifi_autoconnect no
  if wifi_managed && [ "$(dev_state "$WIFI")" = "connected" ]; then
  ...
  # `wifi_managed`, not `has_nm`: on a box whose Wi-Fi belongs to ifupdown every nmcli
  # call here is a no-op, and logging "bringing up wlp6s0" every 30 s while nothing is
  # brought up is worse than silence — it is a log that reads as a working fallback.
  if wifi_managed && [ "$(dev_state "$WIFI")" != "connected" ]; then
...
# ---------------------------------------------------------------------------
# Memory guard: an interactive session must not be able to take the box down.
#
# 2026-08-13 the box was unreachable and hot for 55 minutes and the journal contains
# no OOM kill at all — only `Under memory pressure, flushing caches` 122 times in a row,
# vnstatd taking 380 s to write its database and systemd-timedated killed on timeout.
# A process in user.slice outgrew RAM, the kernel moved the world into 11 GB of swap and
# thrashed. Nothing was ever killed because to the kernel memory stayed "available" — it
# was just on a disk. Swap is what turns one greedy process into a dead box.
#
# Two layers, and only over user.slice: nas-web, docker and the rest sit in system.slice
# and stay uncapped on purpose — the point is to protect the NAS from a session, not to
# ration the NAS.
#   1. A SWAP ceiling (plus a memory ceiling as the outer bound). Capped, the cgroup OOM
#      killer fires in seconds instead of the box degrading for an hour.
#   2. systemd-oomd, which watches memory PRESSURE (PSI) and kills the worst offender
#      before any ceiling is hit. The graceful half; layer 1 is the backstop that works
#      even if the package is missing — the directives below are inert without it.
# ---------------------------------------------------------------------------
install_memory_guard() {
    run mkdir -p /etc/systemd/system/user.slice.d
    write_file /etc/systemd/system/user.slice.d/50-nas-memory.conf <<'MEMG'
# nas-wizard: keep an interactive session from swapping the whole box to death.
# See install_memory_guard() in nas-wizard.sh for the incident this comes from.
[Slice]
MemoryMax=70%
MemorySwapMax=2G
...
MEMG
    # PSI needs a kernel that exports /proc/pressure — every Debian kernel does, but a
    # missing file would make oomd fail to start, so check before pulling the package in.
    if [ -r /proc/pressure/memory ]; then
        install_packages "oomd" systemd-oomd
        if command -v systemd-oomd >/dev/null 2>&1 || [ -x /usr/lib/systemd/systemd-oomd ]; then
            run mkdir -p /etc/systemd/oomd.conf.d
            write_file /etc/systemd/oomd.conf.d/50-nas.conf <<'OOMD'
# nas-wizard: act while the box is still answering, not after it has stalled.
[OOM]
SwapUsedLimit=80%
DefaultMemoryPressureLimit=60%
DefaultMemoryPressureDurationSec=20s
OOMD
            enable_service systemd-oomd
        fi
    else
        info "no /proc/pressure — skipping systemd-oomd, slice limits still apply"
    fi
    run systemctl daemon-reload
    info "memory guard: user.slice capped (swap 2G), oomd watching pressure"
}
...
install_motd() {
    ...
    # Only facts true on EVERY box belong here: the file is written once and then owned
    # by the user, so a line that goes stale (a stacks path that does not exist, a SnapRAID
    # warning on a box without parity) can never be corrected by a later install. Anything
    # conditional is printed by the script below, which re-checks it on every login.
    run mkdir -p /etc/nas-wizard
...
NAS-OS - home NAS
  ...
  Panel        {panel}
  Data pool    /mnt/storage
  Panel logs   journalctl -u nas-web -f
TXT
    fi
    # Boxes installed before the greeting became conditional carry the old default
    # verbatim. Replace it — but ONLY if it is untouched, byte for byte: a user edit
    # outranks our cleanup.
    if [ -f /etc/nas-wizard/motd.txt ] \
       && grep -q 'Stacks  ~/services' /etc/nas-wizard/motd.txt \
       && grep -q 'SnapRAID hates sudden power loss' /etc/nas-wizard/motd.txt \
       && [ "$(wc -l < /etc/nas-wizard/motd.txt)" = "7" ]; then
        write_file /etc/nas-wizard/motd.txt <<'TXT'
NAS-OS - home NAS
...
# ---- logo. NAS-OS brand colours: #C8506B and the indicator green #7BCB59 — the
# same pair web/icon.svg is built from.
...
if [ "${MOTD_LOGO:-1}" = "1" ]; then
  if [ -n "${NO_COLOR:-}" ]; then
    ...
    PIR=$'\033[1;38;2;200;80;107m'; PIG=$'\033[1;38;2;123;203;89m'; PID=$'\033[2;37m'
  else
    PIR=$'\033[1;38;5;168m'; PIG=$'\033[1;38;5;113m'; PID=$'\033[2;37m'
  fi
  printf '\n'
  printf '  %s╔╗╔╔═╗╔═╗%s    %s╔═╗╔═╗%s\n' "$PIR" "$R" "$PIG" "$R"
  printf '  %s║║║╠═╣╚═╗%s%s────%s%s║ ║╚═╗%s\n' "$PIR" "$R" "$PID" "$R" "$PIG" "$R"
...
# ---- power-off warning: ONLY where parity actually exists ------------------
# This used to sit in the greeting text unconditionally, so a box with no SnapRAID
# was warned every login about losing parity it did not have — and a greeting that
# states things that are not true teaches the reader to skim past the ones that are.
# Deliberately NOT behind MOTD_TEXT/MOTD_INFO: it is a data-safety instruction, not a
# status line, and it costs nothing on the boxes that never see it.
if [ -f /etc/snapraid.conf ] && grep -q '^[[:space:]]*parity' /etc/snapraid.conf 2>/dev/null; then
...
# Stacks: the path is only worth naming once something lives there. Counted with a
# glob rather than `ls | wc -l` — motd runs on every login and forks are the one cost
# that matters here.
if [ -d /opt/stacks ]; then
  nst=0
  for d in /opt/stacks/*/; do [ -d "$d" ] && nst=$((nst+1)); done
  [ "$nst" -gt 0 ] && row "Stacks" "/opt/stacks ($nst)"
fi
if [ -n "$V_IP" ]; then
...
EOF
    ...
    install_notify_helper
    # Memory guard before anything heavy runs: a fresh box must not be able to swap
    # itself to death on its very first big job.
    install_memory_guard
    install_motd
    ...
    run mkdir -p "$STORAGE_MNT" "$DOCKER_ROOT" "$SERVICES_SRC"
    if [ ! -d "$NAS_CONFIG" ]; then run mkdir -p "$NAS_CONFIG/scripts"; run chown -R root:root "$NAS_CONFIG"; run chmod 755 "$NAS_CONFIG"; fi
    if [ ! -d "$NAS_CONFIG/.git" ]; then
        run git -C "$NAS_CONFIG" init -q
        nas_config_git_guard          # before the first add -A, not after
        run git -C "$NAS_CONFIG" add -A
