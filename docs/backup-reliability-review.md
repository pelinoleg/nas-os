...
# Проверка правки Codex (полный разбор, перенесён из CLAUDE.md 2026-08-05)

**Проверка чужой работы: правка Codex «Make backup verification failures visible» (2026-07-27).**
Разбор `docs/backup-reliability-review.md` + коммит f74059a. Замысел верный и две дыры настоящие:
слот расписания занимался ДО того, как выяснилось, стартовал ли юнит (синхронный сбой съедал сутки),
а `_kp_drill` молча глотал упавший `kopia restore` — прогон оставался `ok`, хотя восстановление не
доказано. Их тесты (5) и `./check.sh` зелёные, разделение primary/pending-петель у kopia корректно,
формат `_kp_drill` обратно совместим (старые ключи на месте). НАЙДЕНО И ИСПРАВЛЕНО ДВА ДЕФЕКТА:
- **«already running» трактовалось как несостоявшийся старт.** И `nb_run_bg`, и `kp_run_start`
  возвращают такой ответ, когда прогон УЖЕ идёт (ручной запуск, наложившийся на своё расписание).
  Слот при этом не занимался → каждую минуту новая попытка и уведомление «could not start». Это
  ложная тревога о том, что как раз работает. Теперь такой ответ занимает слот наравне с успехом.
- **«нечего было проверять» приравняли к «проверка провалилась».** `_kp_drill_pick` спускается
  случайно и пропускает файлы >200 МБ, поэтому снапшот из одних крупных файлов даёт `attempted=0`;
  условие `not checked` красило такой прогон в `warn` НАВСЕГДА. Теперь warn только при реальном
  провале (`failed`) или расхождении (`rot`), а пустая выборка пишет в лог строку «no file small
  enough to spot-check». ПРАВИЛО: отсутствие данных о проверке и провал проверки — разные вещи;
  путать их значит приучать к жёлтому цвету.
Добавлено 2 регресс-теста (7 всего), прогон `./check.sh py test sh` — ALL CLEAN, состояние
планировщиков после рестарта не изменилось.
