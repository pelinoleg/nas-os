...
## ПЛАН (следующий заход): Kopia — выбор папок деревом с чекбоксами, как в Mirror
Согласовано с пользователем 2026-07-24; НЕ начато. Старые снапшоты/совместимость НЕ важны —
данные тестовые, миграция не нужна.

**Что хотим (семантика ровно как у Mirror `nbPickSource`):** дерево папок с чекбоксами; галка на
папке = вся папка ЦЕЛИКОМ, включая то, что появится в ней позже; развернул выбранную папку, снял
галку с дочерней — исключается ТОЛЬКО она; новые папки рядом с исключённой попадают в бэкап САМИ.
Ключ к этому: ничего не перечисляем заранее — храним «выбрано» + «явные исключения», остальное
выводится.

**Модель состояний узла** (готовая, из `nbPickSource`, переиспользовать 1:1): `checked` (выбран
сам) / `covered` (внутри выбранного) / `excluded` (явно снят) / `exout` (внутри исключённого,
клик игнорируем — сперва вернуть предка) / `indet` (внутри что-то выбрано) / `none`.
Клик: `covered`→добавить в excl; `excluded`→убрать из excl; `checked`→снять выбор и вычистить
его excl-потомков; `none`→выбрать, убрав вложенные sel/excl.

**Данные.** У Source: рядом с текстовыми `excludes[]` (`*.tmp` — остаются) добавить
`exclude_paths[]` — АБСОЛЮТНЫЕ пути. Валидация в `kp_source_save`: каждый путь обязан лежать
ВНУТРИ одной из `folders` (иначе выбросить), realpath, кап ~200, дедуп, выкинуть вложенные друг
в друга исключения (достаточно верхнего).

**Сервер.** `_kp_policy_apply` сейчас валит ВСЕ паттерны во ВСЕ папки — для путей так нельзя:
для каждой папки источника брать только свои `exclude_paths` и превращать в правило, привязанное
к корню политики этой папки: `/media/x/docker` + excl `/media/x/docker/bytestash` → `--add-ignore
"/bytestash/"` (вложенный — `/a/b/`). Текстовые паттерны — как сейчас, во все папки. Сброс правил
уже отдельным вызовом `--clear-ignore` (грабля известна, не ломать).
**ПЕРВЫМ ДЕЛОМ — проверить на ИЗОЛИРОВАННОМ репо**, что ведущий слэш якорит правило к корню
политики (ожидание: gitignore-подобный синтаксис), и что исключённая папка реально не попадает в
снапшот, а НОВАЯ соседняя — попадает. Без этого дальше не идти.

**UI.** Новый `kpPickSource` = копия `nbPickSource`, но листинг локальный (`/api/kopia/browse`,
уже есть; `_NB_ROOT_SKIP` фильтрует мусор) и без «no connection» веток. Встроить в `srcDlg`
вместо плоского «колодца» Folders: кнопка «Choose folders…» открывает дерево (overlay-слой, как
`pickFolder({overlay:true})` — родительский диалог не затирать!), выбранные показываем как сейчас
(`.kp-wpath`: имя + путь), исключённые — чипами/зачёркнутыми под своей папкой. Textarea паттернов
оставить.

**Мелочи, которые не забыть:** (1) дерево структурно снимает вчерашнюю проблему вложенных папок —
`_kp_unnest` на сервере ОСТАВИТЬ как страховку; (2) удалили папку из источника → вычистить её
`exclude_paths`; (3) исключения влияют только на БУДУЩИЕ снапшоты (старые точки не трогаем — так и
надо); (4) счётчик в диалоге: «N folders · M excluded»; (5) тексты — по-английски.

**Тесты:** юнит на конверсию путь→правило (вложенность, пробелы/юникод в именах, путь вне папок
источника); live на изолированном репо (исключённая папка отсутствует в снапшоте, новая соседняя
присутствует); CDP на состояния дерева (covered→excluded→восстановление, exout не кликается,
счётчик, сохранение и перечитывание из конфига).
