...
- **ХВОСТ ОТ СТАРОГО**: в `/opt/stacks/syncthing/` лежит docker-стек с pi5 (`web-desktop.url=
  http://pi5.local:8384`, sidecar клонирования Vellum). Контейнера НЕТ, но если его поднять —
  конфликт портов 8384/22000 с системной службой и второй ярлык на столе. Не удалял (данные
  пользователя) — снести можно из приложения Docker.
