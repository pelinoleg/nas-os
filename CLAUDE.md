...
- **ХВОСТ, который остался у пользователя:** в бэкапе всё ещё лежит рваная копия ЖИВОЙ базы
  `Ugreen-Backup/docker/immich/postgres` (1.3 ГБ, последняя запись 22 июля, до добавления
  исключения). Восстанавливать из неё нельзя, а выглядит она как база — ловушка на 3 часа ночи.
  rsync сам её не удалит (исключённое, уже скопированное, удаляется только с
  `--delete-excluded`). Снести руками: `sudo rm -rf "/media/nas/t7-4TB/Ugreen-Backup/docker/immich/postgres"`.
...
- **Побочные находки:** (1) в бэкапе лежит 1.3 ГБ «живой» папки `docker/immich/postgres` —
  рваная копия работающей БД, снятая ДО того, как в исключения добавили `immich/postgres/`;
  rsync не удаляет уже скопированное исключённое (нужен `--delete-excluded`), самый свежий файл
  внутри от 22 июля. Восстанавливать из неё нельзя, а выглядит она как база — удалить.
