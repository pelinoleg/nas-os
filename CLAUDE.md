...
- **Побочные находки:** (1) в бэкапе лежит 1.3 ГБ «живой» папки `docker/immich/postgres` —
  ...
  (2) На Ugreen в `.env` Immich `IMMICH_VERSION` ЗАКОММЕНТИРОВАН (= `release`), то есть при подъёме
  «как на источнике» приедет последняя версия, которая может не принять схему v3.0.1; в НАШЕМ
  рецепте каталога версия прибита (`IMMICH_VERSION=v3.0.1`) — поднимать надо из него.
